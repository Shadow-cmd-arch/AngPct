...
def agrandir(event, bouton):
    bouton.config(padx=2, pady=1,bg="#c48764")

def reduire(event, bouton):
    bouton.config(padx=0, pady=0,bg="#c67242")
...
def set_language(langue):
    ...
    menu_bar.entryconfig(menu_theme_index, label=current_translations["theme"])
    menu_bar.entryconfig(0,Label=current_translations["dark"])
    menu_bar.entryconfig(1,Label=current_translations["light"])

def dark():
    fenetre.config(background="#00031d")
    titre.config(background="#00031d") 
    pourcentages.config(background="#00031d")
    Degré.config(background="#00031d")
    vers.config(background="#00031d")
    vers.config(foreground="#1c4750",background="#00031d")

def light():
    fenetre.config(background="#F8FAF9") 
    titre.config(background="#F8FAF9")
    pourcentages.config(background="#F8FAF9")
    Degré.config(background="#F8FAF9")
    vers.config(background="#F8FAF9")
    vers.config(background="#F8FAF9")
    vers.config(foreground="#000000",background="#F8FAF9")
...
def chance2():
    ...
    B1=Button(quitter,text="OUI",font=("Arial",15),bg="#c67242",fg="black",width=15,command=oui)
    B2=Button(quitter,text="NON",font=("Arial",15),bg="#c67242",fg="black",width=15,command=non)

    titre.place(x=15,y=5)
    B1.place(x=30,y=50)
    B2.place(x=210,y=50)
    ...
    B1.bind("<Enter>", lambda event: agrandir(event, B1))
    B1.bind("<Leave>", lambda event: reduire(event, B1))

    B2.bind("<Enter>", lambda event: agrandir(event, B2))
    B2.bind("<Leave>", lambda event: reduire(event, B2))
...
fenetre.title("AngPct   vers: 1.3")#________________________________________________________________________________________Titre de la fenêtre
...
menu_language.add_command(label="العربية", command=lambda: set_language("ar"))
menu_language.add_command(label="Italiano", command=lambda: set_language("it"))
menu_language.add_command(label="中文", command=lambda: set_language("zh"))
menu_language.add_command(label="Português", command=lambda: set_language("pt"))
...
menu_theme=Menu(menu_bar, tearoff=0)
menu_theme.add_command(label="Dark", command=dark)
menu_theme.add_command(label="Light", command=light)
menu_bar.add_cascade(label="Theme", menu=menu_theme)
menu_theme_index = menu_bar.index("Theme")
...
convertir_bouton=Button(text="Convertir",font=("Cylburn",15),bg="#c67242",fg="#000000",width=0,height=0,command=convertir)#_____ Bouton 'Convertir'.
...
vers = Label(fenetre,text="Vers: 1.3",font=("Arial",15),fg="black",bg="#ffffff")
...
convertir_bouton.bind("<Enter>", lambda event: agrandir(event, convertir_bouton))
convertir_bouton.bind("<Leave>", lambda event: reduire(event, convertir_bouton))

convertir_bouton_2.bind("<Enter>", lambda event: agrandir(event, convertir_bouton_2))
convertir_bouton_2.bind("<Leave>", lambda event: reduire(event, convertir_bouton_2))

Bouton_quitter.bind("<Enter>", lambda event: agrandir(event, Bouton_quitter))
Bouton_quitter.bind("<Leave>", lambda event: reduire(event, Bouton_quitter))

bouton_effacer.bind("<Enter>", lambda event: agrandir(event, bouton_effacer))
bouton_effacer.bind("<Leave>", lambda event: reduire(event, bouton_effacer))
